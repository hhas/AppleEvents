...
/*
 typealias Property = (key: DescType, value: Descriptor)
 
 private var items = [Property]()
 
 public mutating func add(key: DescType, value: Descriptor) {
 ...
 public func appendTo(containerData result: inout Data) {
 // appends this record to a list (item), record (property value), or event (attribute/parameter value)
 /* nested records always use short form:
 ...
 */
 ...
 result += packUInt32(self.type)
 let idx = result.count
 result += Data([0, 0, 0, 0]) // remaining bytes (TBC)
 result += packUInt32(UInt32(self.items.count)) // number of items
 result += Data([0, 0, 0, 0]) // align
 for (key, value) in self.items {
 result += packUInt32(key)
 value.appendTo(containerData: &result)
 }
 result[idx..<idx+4] = packUInt32(UInt32(result.count - idx - 4)) // remaining bytes
 }
